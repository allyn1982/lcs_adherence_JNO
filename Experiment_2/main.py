...
def main(args=None):
    ...
    parser.add_argument('--train_data_type',
                        help='Must be "full_complete", "full_missing", "simple_complete", or "simple_missing". If testing final full model, use "full_complete". If testing final simple model, use "simple_imputed"')
    ...
    # Note: in cv_outer, if using dummy data, use n_splits=3 to avoid getting an error for roc-auc: 'Only one class present in y_true. ROC AUC score is not defined in that case.'
    ...
    # cross validation on complete data sets
    ...
    if parser.cv_or_test == 'test' and parser.train_data_type == 'full_complete':
        test_results(classifier=LogisticRegression(),
                     model_type='full',
                     file_path=full_complete_path,
                     file_type='full_complete',
